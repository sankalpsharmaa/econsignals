...
# File lives at: <proj_root>/.claude/skills/econsignals/lib/relevance.py
# parents[0] = lib
# parents[1] = econsignals  (skill root)
# parents[2] = skills
# parents[3] = .claude
# parents[4] = econsignals project root
SKILL_ROOT: Path = Path(__file__).resolve().parent.parent
PROJ_ROOT: Path = SKILL_ROOT.parent.parent.parent
...
def score_all_papers(days: int = 7) -> int:
    ...
    from lib.db import get_db, get_recent_papers, update_paper_relevance  # noqa: PLC0415
...
def update_jel_weights(paper_id: int, is_interesting: bool) -> None:
    ...
    import sys
    sys.path.insert(0, str(SKILL_ROOT))
    from lib.db import get_db  # noqa: PLC0415
...
if __name__ == "__main__":
    import sys
    sys.path.insert(0, str(SKILL_ROOT))
    from lib.db import init_db  # noqa: PLC0415

...
# File lives at:  <proj_root>/.claude/skills/econsignals/lib/db.py
# parents[0] = lib
# parents[1] = econsignals
# parents[2] = skills
# parents[3] = .claude
# parents[4] = econsignals project root
PROJ_ROOT: Path = Path(__file__).resolve().parents[4]
...
def get_db() -> sqlite3.Connection:
    ...
    Calls init_db() on first use so callers never need to manage
...
def log_sensor_start(sensor: str, watch: str) -> int:
    ...
    Args:
        sensor: Sensor name (e.g. "arxiv", "ssrn").
